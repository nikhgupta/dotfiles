...
OptionParser.new do |parser|
 parser.on('--copy') { |val| options.merge!(copy: true, backup: false) }
 parser.on('-n', '--dry-run') { |val| options[:dry_run] = true }
 parser.on('-v', '--verbose') { |val| options[:verbose] = true }
 parser.on('--no-backup') { |val| options[:backup] = false}
 parser.on('-h', '--help') do
   puts "Usage:"
...
if options[:copy] && options[:backup]; then
