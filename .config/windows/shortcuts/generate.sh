...
win_shortcut.sh $_dir/vim $_dir/icons/vim.ico $WIN_HOME/AppData/Local/wsltty/bin/mintty.exe "CTRL+ALT+]" --WSL= --configdir=$wsltty_config -w full -t NeoVim nvim
...
win_shortcut.sh $_dir/play $_dir/icons/play.ico $WIN_HOME/AppData/Local/wsltty/bin/mintty.exe "CTRL+ALT+\\" -o Padding=1 --WSL= --configdir=$wsltty_config -w full -t MusicPlayer ncmpcpp
