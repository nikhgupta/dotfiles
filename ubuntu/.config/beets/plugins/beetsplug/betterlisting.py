...
def config_icon(item, view):
    ...
    if view:
        try:
            fmt = (b"\\U%08x" % view.get(int)).decode('unicode-escape')
        except:
            fmt = view.get(str)
...
class BetterListingPlugin(plugins.BeetsPlugin):
    ...
    def tmpl_sparkbar(self, count, total, color=u''):
        ...
        sparks = self.config["sparks"].get(str)[::-1]
...
for prefix, name in BetterListingPlugin._prefixes.items():
